...
function loadState(sessionId: string, prefs: SidebarPrefs): SidebarState {
  ...
  // Powerdesk ships no explorer view (the sidebar hosts only the terminal +
  // browser tabs), so never seed the reference's default explorer tab — a
  // fresh session starts with an empty pane whose + menu offers the
  // registered surfaces. The `seedExplorer` arg is forced false regardless
  // of the (inert) `tabsEnabled['explorer']` pref.
  return makeDefaultState(width, openByDefault, false)
}

...
export class ResttyError extends Error {
  constructor(
...
export function writeJson(res: ResttyHttpResponse, status: number, body: unknown): void {
...
export function writeOk(res: ResttyHttpResponse, value: unknown): void {
  writeJson(res, 200, { ok: true, value })
}

/** Write the failure envelope for any thrown value (unknown → internal 500). */
export function writeError(res: ResttyHttpResponse, error: unknown): void {
...
export function requireString(payload: unknown, key: string): string {
  ...
  if (typeof value !== 'string' || value === '') {
    throw new ResttyError('bad-request', `missing or invalid "${key}"`)
  }
  return value
}

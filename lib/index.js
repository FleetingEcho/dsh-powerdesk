import { a as MANIFEST_FILE, c as readExtension, d as ExtensionError, f as parseManifest, i as INSTALL_RECORD_FILE, l as removeExtension, o as extensionDir, r as registerBundleRoute, s as listExtensions, u as resolveExtensionsDir } from "./bundle-route-C2ZilHdc.js";
...
function latin1(buf, offset, length) {
	let out = "";
	for (let i = offset; i < offset + length && i < buf.length; i += 1) out += String.fromCharCode(buf[i]);
	return out;
}
/** Decode a NUL/space-terminated header string field. */
function headerString(buf, offset, length) {
...
*/
function headerOctal(buf, offset, length, field) {
	if ((buf[offset] & 128) !== 0) throw new TarError(`tar header field "${field}" uses the unsupported base-256 encoding`);
	const text = headerString(buf, offset, length).trim();
	if (text === "") return 0;
	if (!/^[0-7]+$/.test(text)) throw new TarError(`tar header field "${field}" is not octal`);
...
function checksumOk(block) {
	...
	for (let i = 0; i < BLOCK; i += 1) {
		const byte = i >= 148 && i < 156 ? 32 : block[i];
		unsigned += byte;
...
function safeEntryPath(raw) {
	if (raw === "" || raw.includes("\0") || raw.includes("\\")) return void 0;
	if (raw.startsWith("/") || /^[A-Za-z]:/.test(raw)) return void 0;
	const segments = [];
...
const REGULAR_TYPES = /* @__PURE__ */ new Set([
	"0",
	"\0",
	"7"
]);
...
const REJECTED_TYPES = /* @__PURE__ */ new Map([
	["1", "hard link"],
	["2", "symbolic link"],
	["3", "character device"],
	["4", "block device"],
...
*/
function untar(buf, limits = DEFAULT_UNTAR_LIMITS) {
	const entries = [];
	let total = 0;
	let headers = 0;
	let offset = 0;
	...
	while (offset + BLOCK <= buf.length) {
		...
		if (!checksumOk(block)) throw new TarError("tar header checksum mismatch (truncated or not a tar archive)");
		headers += 1;
		if (headers > limits.maxEntries) throw new TarError(`tar archive has more than ${limits.maxEntries} members`);
		const size = headerOctal(block, 124, 12, "size");
		if (size > limits.maxEntryBytes) throw new TarError(`tar member exceeds the ${limits.maxEntryBytes}-byte per-file limit`);
		const dataOffset = offset + BLOCK;
		if (dataOffset + size > buf.length) throw new TarError("tar archive is truncated (member data runs past the end)");
		...
		if (rejected !== void 0) throw new TarError(`tar member "${headerName(block, pendingName)}" is a ${rejected}; extension archives may only contain regular files`);
		const rawName = headerName(block, pendingName);
		pendingName = void 0;
		offset = dataOffset + roundUp(size);
		if (type === "5" || rawName.endsWith("/")) {
			if (safeEntryPath(rawName) === void 0) throw new TarError(`tar member "${rawName}" has an unsafe path`);
			continue;
		}
		if (!REGULAR_TYPES.has(type)) throw new TarError(`tar member "${rawName}" has unsupported type "${type}"`);
		const path = safeEntryPath(rawName);
		if (path === void 0) throw new TarError(`tar member "${rawName}" has an unsafe path`);
		total += size;
		if (total > limits.maxTotalBytes) throw new TarError(`tar archive expands beyond the ${limits.maxTotalBytes}-byte total limit`);
		entries.push({
			path,
			data: buf.slice(dataOffset, dataOffset + size)
		});
	}
	return entries;
}
...
function headerName(block, pendingName) {
	if (pendingName !== void 0 && pendingName !== "") return pendingName;
...
function stripCommonRoot(entries) {
	...
	if (new Set(entries.map((entry) => entry.path.split("/")[0])).size !== 1) return [...entries];
...
//#endregion
//#region src/extensions/install.ts
/**
...
const MAX_UPLOAD_BYTES = 16777216;
/**
...
*/
const MAX_INFLATED_BYTES = DEFAULT_UNTAR_LIMITS.maxTotalBytes;
/** gzip member magic. */
...
function inflate(buf) {
	try {
		...
		if (/buffer|length|too large/i.test(message)) throw new ExtensionError(`archive expands beyond the ${MAX_INFLATED_BYTES}-byte limit`);
...
function decode(data) {
	...
	if (looksLikeTar(raw)) return {
		kind: "tar",
		entries: stripCommonRoot(untar(raw))
	};
	return {
		kind: "bare",
		data: raw
...
function stageTar(entries) {
	const manifestEntry = entries.find((entry) => entry.path === MANIFEST_FILE);
	if (manifestEntry === void 0) throw new ExtensionError(`archive has no ${MANIFEST_FILE} at its root`);
	let parsed;
	try {
		parsed = JSON.parse(Buffer.from(manifestEntry.data).toString("utf8"));
	} catch (error) {
		throw new ExtensionError(`${MANIFEST_FILE} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
	}
	const manifest = parseManifest(parsed);
	if (!entries.some((entry) => entry.path === manifest.entry)) throw new ExtensionError(`archive has no "${manifest.entry}" (the manifest's entry file)`);
	return {
		manifest,
		files: entries.filter((entry) => entry.path !== INSTALL_RECORD_FILE)
	};
}
/** Build the staged file set from a bare bundle upload. */
function stageBare(data, fallback) {
	if (fallback === void 0) throw new ExtensionError(`upload is a single script, not an archive — give it an id and a title, or upload a .tgz containing ${MANIFEST_FILE}`);
	const manifest = parseManifest({
		apiVersion: 1,
		id: fallback.id,
		title: fallback.title,
		...fallback.icon !== void 0 ? { icon: fallback.icon } : {},
		entry: "bundle.js"
	});
	return {
		manifest,
...
*/
async function installExtension(root, upload) {
	if (upload.data.length === 0) throw new ExtensionError("upload is empty");
	if (upload.data.length > 16777216) throw new ExtensionError(`upload exceeds the ${MAX_UPLOAD_BYTES}-byte limit`);
	const payload = decode(upload.data);
	const staged = payload.kind === "tar" ? stageTar(payload.entries) : stageBare(payload.data, upload.fallback);
	const record = {
	...
	if (installed.error !== void 0) throw new ExtensionError(`installed extension is not readable: ${installed.error}`);
	return installed;
}
//#endregion
//#region src/shell.ts
...
async function readJsonBody(req, maxBytes = MAX_BODY_BYTES) {
	...
	for await (const chunk of req) {
		...
		total += buffer.length;
		if (total > maxBytes) throw new ResttyError("bad-request", "request body too large");
		chunks.push(buffer);
	}
...
const defaultRequire$1 = createRequire(import.meta.url);
let cached$2;
/** The napi binary file name the build produces (napi-rs default naming). */
const NATIVE_BASENAME$1 = "dsh_powerdesk_pty.node";
/**
...
function nativeCandidates(triple, pluginRoot, env = process.env) {
	...
	if (pluginRoot !== null) candidates.push(join(pluginRoot, "prebuilt", triple, NATIVE_BASENAME$1));
	return candidates;
}
/**
...
*/
function loadRustPty(requireImpl = defaultRequire$1) {
	if (cached$2 === void 0) {
...
function loadRequiredRustPty() {
	...
	if (module === null) {
		const cause = describeCause$1(rustPtyLoadCause());
...
function describeCause$1(cause) {
...
function depsStatus(options = {}) {
	...
	return {
		ok: false,
		cause: describeCause$1(rustPtyLoadCause()),
		command,
...
const RIPGREP_BASENAME = (platform = process.platform) => platform === "win32" ? "rg.exe" : "rg";
let cached$1;
/**
...
*/
function ripgrepCandidates(triple, pluginRoot, env = process.env, platform = process.platform) {
	const candidates = [];
...
const defaultRipgrepProber = (spec) => {
	try {
...
*/
function resolveRipgrepPath(prober = defaultRipgrepProber) {
	if (cached$1 === void 0) {
		const triple = detectPlatformTriple();
		const candidates = ripgrepCandidates(triple, findPluginRoot());
		let result = {
			ok: false,
			cause: /* @__PURE__ */ new Error(`no ripgrep candidate for triple "${triple}"`)
		};
		for (const spec of candidates) if (prober(spec)) {
			result = {
				ok: true,
				path: spec
			};
	...
	return cached$1.ok ? cached$1.path : null;
}
/** The recorded resolution failure (undefined once a candidate resolves). */
function ripgrepLoadCause() {
	return cached$1 !== void 0 && !cached$1.ok ? cached$1.cause : void 0;
}
/** Current ripgrep dependency status (resolved vs missing + repair info). */
function searchDepsStatus(options = {}) {
	...
	return {
		...
		...note !== void 0 ? { note: "ripgrep could not be located. Run scripts/install.sh --repair (re-downloads the platform prebuilt), then restart DSH." } : {}
	};
}
//#endregion
//#region src/search-api.ts
/**
...
async function searchGrep(path, query, options = {}) {
	if (query.trim() === "") return {
		files: [],
		truncated: false
	};
	const rg = resolveRipgrepPath();
	if (rg === null) throw new ResttyError("search-deps-missing", "ripgrep is not available — run the repair command shown in the Search tab", 503);
	return runRipgrep(rg, resolve(path), query, options);
}
function runRipgrep(rg, cwd, query, options) {
	return new Promise((resolvePromise, reject) => {
		...
		const buildResult = () => ({
			files: [...files.entries()].map(([filePath, matches]) => ({
				path: filePath,
				matches
			})),
			truncated
		...
		child.stdout.on("data", (chunk) => {
			...
			while ((newlineIdx = buffer.indexOf("\n")) >= 0) {
				...
				list.push({
					...
					ranges: match.data.submatches.map((s) => [s.start, s.end])
				});
		...
		child.on("error", (error) => {
			settle({
				ok: false,
				error: new ResttyError("bad-request", `ripgrep failed to start: ${error.message}`, 400)
			});
		});
		child.on("close", (code) => {
			if (code === 2) {
				...
				settle({
					ok: false,
					error: new ResttyError("bad-request", message, 400)
				});
				return;
			}
			settle({ ok: true });
...
//#endregion
//#region src/rust-sqlite-deps.ts
/**
...
const DSH_POWERDESK_SQLITE_VERSION = "0.1.0";
...
*/
function detectSqliteTriple(platform = process.platform, arch = process.arch, env = process.env) {
	const override = env.DSH_POWERDESK_SQLITE_TRIPLE;
	return detectPlatformTriple(platform, arch, {
		...env,
...
* Exposed for tests.
*/
function sqliteCandidates(triple, pluginRoot, env = process.env) {
	const candidates = [];
...
*/
function loadRustSqlite(requireImpl = defaultRequire) {
	if (cached === void 0) {
		const triple = detectSqliteTriple();
		const candidates = sqliteCandidates(triple, findPluginRoot());
		let result = {
			ok: false,
			cause: /* @__PURE__ */ new Error(`no native sqlite candidate for triple "${triple}"`)
		};
		for (const spec of candidates) try {
			const mod = requireImpl(spec);
			const surface = (mod?.Database !== void 0 ? mod : mod.default) ?? mod;
			if (surface !== void 0 && typeof surface.Database?.open === "function") {
				result = {
					ok: true,
					module: surface
				};
			...
			result = {
				ok: false,
				cause: /* @__PURE__ */ new Error(`native module "${spec}" has no Database.open export`)
			};
		} catch (cause) {
			result = {
				ok: false,
...
function loadRequiredRustSqlite() {
	const module = loadRustSqlite();
	if (module === null) {
		const cause = describeCause(rustSqliteLoadCause());
		throw new ResttyError("sqlite-deps-missing", `dsh-powerdesk-sqlite (${DSH_POWERDESK_SQLITE_VERSION}) failed to load: ${cause} — run the repair command shown in the calendar tab`, 503);
	}
	return module;
}
/** One-line human description of the recorded load cause. */
function describeCause(cause) {
	if (cause instanceof Error) return cause.message;
	if (cause === void 0) return "native module not found";
	return String(cause);
}
/** Current native sqlite dependency status (loaded vs degraded + repair info). */
function sqliteDepsStatus(options = {}) {
	if (loadRustSqlite() !== null) return { ok: true };
	const pluginRoot = findPluginRoot(options.fromFile);
	const profileDir = findProfileDir(options.fromFile);
	const { command, note } = buildRepairCommand({
		pluginRoot,
	...
	return {
		...
		command,
		profile: profileDir !== null ? profileDir.split(/[/\\]/).pop() ?? null : null,
		...note !== void 0 ? { note: "the SQLite native binary could not be located. Run scripts/install.sh --repair (re-downloads the platform prebuilt) or bash scripts/build-rust-sqlite.sh (builds from source with a Rust toolchain), then restart DSH." } : {}
	};
}
//#endregion
//#region src/calendar-api.ts
/**
...
function migrateSchema(db) {
	const existing = new Set(db.query("PRAGMA table_info(calendar_events)").map((c) => c.name));
	for (const [name, type] of Object.entries(ADDED_COLUMNS)) if (!existing.has(name)) db.exec(`ALTER TABLE calendar_events ADD COLUMN ${name} ${type}`);
}
...
function dbPath() {
	...
	if (profileDir === null) throw new ResttyError("calendar-no-profile", "could not locate the DSH profile directory for the calendar database", 500);
...
function getDb() {
	...
	const db = loadRequiredRustSqlite().Database.open(dbPath());
	db.exec(SCHEMA_SQL);
	migrateSchema(db);
	dbHandle = db;
	return db;
...
function requireEventFields(input) {
	if (typeof input.id !== "string" || input.id === "") throw new ResttyError("bad-request", "calendar event \"id\" is required (non-empty string)");
	if (typeof input.start !== "string" || input.start === "") throw new ResttyError("bad-request", "calendar event \"start\" is required (ISO datetime string)");
	if (typeof input.end !== "string" || input.end === "") throw new ResttyError("bad-request", "calendar event \"end\" is required (ISO datetime string)");
}
/** List all events, earliest first. */
function calendarList() {
	return { events: getDb().query("SELECT id, title, start, end, location, description, calendar_id, color, tag FROM calendar_events ORDER BY start ASC").map(rowToEvent) };
}
/** Create an event. Returns the created event. */
function calendarCreate(input) {
	requireEventFields(input);
	getDb().run("INSERT INTO calendar_events (id, title, start, end, location, description, calendar_id, color, tag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
		input.id,
		input.title ?? null,
		input.start,
		input.end,
...
function calendarUpdate(input) {
	if (typeof input.id !== "string" || input.id === "") throw new ResttyError("bad-request", "calendar event \"id\" is required for update");
	const existing = getDb().query("SELECT id, title, start, end, location, description, calendar_id, color, tag FROM calendar_events WHERE id = ?", [input.id])[0];
	if (existing === void 0) return { changes: 0 };
	const merged = {
		id: existing.id,
	...
	return { changes: getDb().run("UPDATE calendar_events SET title = ?, start = ?, end = ?, location = ?, description = ?, calendar_id = ?, color = ?, tag = ?, updated_at = datetime('now') WHERE id = ?", [
		merged.title,
		merged.start,
		merged.end,
		merged.location,
...
function calendarDelete(id) {
	if (typeof id !== "string" || id === "") throw new ResttyError("bad-request", "calendar event \"id\" is required for delete");
	return { changes: getDb().run("DELETE FROM calendar_events WHERE id = ?", [id]) };
}
//#endregion
//#region src/index.ts
...
*/
function bodyLimitFor(method) {
	return method === "ext.install" ? Math.ceil(MAX_UPLOAD_BYTES * 4 / 3) + 65536 : MAX_BODY_BYTES;
}
/** Translate an extension-layer rejection into the API's error envelope. */
function asResttyError(error) {
...
function buildApi(ctx, ptyManager, extensions) {
	/** Refuse extension mutations while the feature is off in config. */
	const requireExtensionsEnabled = () => {
		if (!extensions.enabled) throw new ResttyError("forbidden", "extensions are disabled; set extensionsEnabled in the dsh-powerdesk plugin config", 403);
	};
	const cwdOf = (payload) => {
	...
	return {
		...
		"calendar.create": (payload) => {
			return calendarCreate(payload ?? {});
		},
		"calendar.update": (payload) => {
			return calendarUpdate(payload ?? {});
		},
		...
		"ext.install": async (payload) => {
			...
			const record = payload;
			if (typeof record.dataBase64 !== "string" || record.dataBase64 === "") throw new ResttyError("bad-request", "missing or invalid \"dataBase64\"");
			const data = new Uint8Array(Buffer.from(record.dataBase64, "base64"));
			if (data.length === 0) throw new ResttyError("bad-request", "\"dataBase64\" did not decode to any bytes");
...
function apply(ctx, config) {
	...
	if (extensions.enabled) console.warn(`[dsh-powerdesk] user extensions ENABLED; loading from ${extensions.dir}. Extensions run with full page privileges.`);
...
//#endregion
export { Config, apply, bodyLimitFor, inject, name };

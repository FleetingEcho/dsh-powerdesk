...
function calendarList() {
	return { events: getDb().query("SELECT id, title, start, end, location, description, calendar_id FROM calendar_events ORDER BY start ASC").map(rowToEvent) };
}
/** Create an event. Returns the created event. */
function calendarCreate(input) {
	requireEventFields(input);
	getDb().run("INSERT INTO calendar_events (id, title, start, end, location, description, calendar_id) VALUES (?, ?, ?, ?, ?, ?, ?)", [
		input.id,
		input.title ?? null,
		input.start,
		input.end,
...
function calendarUpdate(input) {
	...
	const existing = getDb().query("SELECT id, title, start, end, location, description, calendar_id FROM calendar_events WHERE id = ?", [input.id])[0];
	...
	return { changes: getDb().run("UPDATE calendar_events SET title = ?, start = ?, end = ?, location = ?, description = ?, calendar_id = ?, updated_at = datetime('now') WHERE id = ?", [
		merged.title,
		merged.start,
		merged.end,
		merged.location,

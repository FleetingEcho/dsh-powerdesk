...
globalThis.__dshPowerdeskChunks__["calendar"] = (require) => {
	...
	*/
	...
	const LOCAL_TIMEZONE = import_temporal_polyfill.Temporal.Now.timeZoneId();
	/**
	...
	*/
	function toTemporal(iso) {
		if (iso.includes("T")) return import_temporal_polyfill.Temporal.ZonedDateTime.from(`${iso}[${LOCAL_TIMEZONE}]`);
		return import_temporal_polyfill.Temporal.PlainDate.from(iso);
	}
	/**
	...
	*/
	function toIso(value) {
		if (value instanceof import_temporal_polyfill.Temporal.ZonedDateTime) return value.toPlainDateTime().toString();
	...
	*  hour". Uses local getters (not toISOString, which would give UTC). */
	...
	function plusOneHour(iso) {
		return import_temporal_polyfill.Temporal.ZonedDateTime.from(`${iso}[${LOCAL_TIMEZONE}]`).add({ hours: 1 }).toPlainDateTime().toString();
	}
